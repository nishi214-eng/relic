L.Control.SlideMenu = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    if (this._isHorizontal) {
      ...
      if (this._isLeftPosition) {
        var width = parseInt(this.options.width, 10) * 2;
        this._menu.style.left = "-" + width + "%";
      } else {
        var width = parseInt(this.options.width, 10) * 2;
        this._menu.style.right = "-" + width + "%";
      }
  ...
  _animate: function (menu, from, to, isOpen, ispx, unit) {
    if (this._isHorizontal) {
      if (this._isLeftPosition) {
        menu.style.left = from * 2 + (ispx ? "px" : "%");
      } else {
        menu.style.right = from * 2 + (ispx ? "px" : "%");
      }
    } else {
      if (this._isTopPosition) {
